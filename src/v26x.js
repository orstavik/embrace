import { LoopCube } from "./LoopCube26.js";
...
class EmbraceRoot {
  constructor(name, docFrag, nodes, expressions) {
    ...
    this.templateElement = null; // To be set by parseTemplate
    this.nodes = nodes;
...
function* flatDomNodesAll(docFrag) {
  ...
  it.nextNode(); // Skip the root DocumentFragment itself
  let n;
  while (n = it.nextNode()) {
    yield n;
    if (n instanceof Element) {
      for (let attr of n.attributes) {
        if (!attr.name.startsWith("embrace_")) {
          yield attr;
        }
      }
    }
...
function parseTemplate(template, name = "embrace") {
  ...
  const root = new EmbraceRoot(name, template.content, nodes, expressions);
  root.templateElement = template;
...
function parseNode(n, name) {
  if (n instanceof Text || n instanceof Attr || n instanceof Comment) {
    if (n.textContent.match(/{{([^}]+)}}/)) {
      const parent = n.ownerElement || n.parentElement;
      if (parent && parent.setAttribute) parent.setAttribute(name, "");
...
// :embrace
export function embrace(ownerElement, dataObject) {
  let em = ownerElement.__embrace;
  if (em)
    return em.run(dotScope(dataObject), 0, ownerElement);
  const id = "embrace_" + ownerElement.id;
  ownerElement.setAttribute(id, "");
  const templ = ownerElement.firstElementChild;
  if (!(templ instanceof HTMLTemplateElement))
    throw new Error("This first element child of :embrace ownerElement must be a template");
  em = ownerElement.__embrace = parseTemplate(templ, id);
  if (window.Embrace?.[id])
    return em.runFirst(ownerElement, dataObject, window.Embrace[id]);
  const funcs = extractFuncs(em);
  const script = "{\n" + Object.entries(funcs).map(([k, v]) => `/* ==================== JS: ${k} ==================== */\n  ${k}: ${v}`).join(',\n\n') + "\n}";
  DoubleDots.importBasedEval(script).then(funcs => {
    console.log?.(":embrace production", hashDebug(script, id));
    (window.Embrace ??= {})[id] = funcs;
    em.runFirst(ownerElement, dataObject, funcs);
  });
}
